...
impl HyperlightVm {
    ...
    pub(crate) fn new(
        ...
        let mut vm: VmType = match get_available_hypervisor() {
            #[cfg(kvm)]
            Some(HypervisorType::Kvm) => Box::new(KvmVm::new().map_err(|e| new_error!("{:?}", e))?),
            #[cfg(mshv3)]
            Some(HypervisorType::Mshv) => Box::new(MshvVm::new().map_err(|e| new_error!("{:?}", e))?),
            #[cfg(target_os = "windows")]
            Some(HypervisorType::Whp) => Box::new(WhpVm::new().map_err(|e| new_error!("{:?}", e))?),
            None => return Err(HyperlightError::NoHypervisorFound()),
        };
        ...
        for (i, region) in mem_regions.iter().enumerate() {
            ...
            unsafe { vm.map_memory((i as u32, region)).map_err(|e| new_error!("{:?}", e))? };
        }
        crate::timing!("[TIMING]   HyperlightVm::new: map_memory ({} regions): {:?}", mem_regions.len(), tv1.elapsed());

        let tv2 = std::time::Instant::now();
        #[cfg(feature = "init-paging")]
        vm.set_sregs(&CommonSpecialRegisters::standard_64bit_defaults(_pml4_addr)).map_err(|e| new_error!("{:?}", e))?;
        #[cfg(not(feature = "init-paging"))]
        vm.set_sregs(&CommonSpecialRegisters::standard_real_mode_defaults()).map_err(|e| new_error!("{:?}", e))?;
        ...
        if ret.gdb_conn.is_some() {
            ...
            ret.vm.set_debug(true).map_err(|e| new_error!("{:?}", e))?;
            ret.vm.add_hw_breakpoint(entrypoint).map_err(|e| new_error!("{:?}", e))?;
        }
    ...
    pub(crate) fn initialise(
        ...
        self.vm.set_regs(&regs).map_err(|e| new_error!("{:?}", e))?;
    ...
    pub(crate) unsafe fn map_region(&mut self, region: &MemoryRegion) -> Result<()> {
        ...
        unsafe { self.vm.map_memory((slot, region)).map_err(|e| new_error!("{:?}", e))? };
    ...
    pub(crate) fn unmap_region(&mut self, region: &MemoryRegion) -> Result<()> {
        ...
        self.vm.unmap_memory((slot, region)).map_err(|e| new_error!("{:?}", e))?;
        Ok(())
    }
    ...
    pub(crate) fn dispatch_call_from_host(
        ...
        // reset fpu
        self.vm.set_fpu(&CommonFpu::default()).map_err(|e| new_error!("{:?}", e))?;
        ...
        let result = loop {
            ...
            match exit_reason {
                ...
                Ok(VmExit::Debug { dr6, exception }) => {
                    ...
                    let stop_reason =
                        arch::vcpu_stop_reason(self.vm.as_mut(), dr6, self.entrypoint, exception).map_err(|e| new_error!("{:?}", e))?;
    ...
    pub(crate) fn crashdump_context(&self) -> Result<Option<super::crashdump::CrashDumpContext>> {
        if self.rt_cfg.guest_core_dump {
            ...
            let vcpu_regs = self.vm.regs().map_err(|e| new_error!("{:?}", e))?;
            let sregs = self.vm.sregs().map_err(|e| new_error!("{:?}", e))?;
            let xsave = self.vm.xsave().map_err(|e| new_error!("{:?}", e))?;
...
mod debug {
    ...
    impl HyperlightVm {
            ...
            if self.gdb_conn.is_some() {
                match req {
                    ...
                    DebugMsg::ReadRegisters => {
                        let regs = self.vm.regs().map_err(|e| new_error!("{:?}", e))?;
                        let fpu = self.vm.fpu().map_err(|e| new_error!("{:?}", e))?;
                    ...
                    DebugMsg::WriteRegisters(boxed_regs) => {
                        ...
                        self.vm.set_regs(regs).map_err(|e| new_error!("{:?}", e))?;
                        self.vm.set_fpu(fpu).map_err(|e| new_error!("{:?}", e))?;
        ...
        fn read_addrs(
            ...
            while !data.is_empty() {
                let gpa = self.vm.translate_gva(gva).map_err(|e| new_error!("{:?}", e))?;
