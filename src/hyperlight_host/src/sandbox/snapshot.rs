...
pub struct Snapshot {
    ...
    hash: [u8; 32],
    /// The address of the root page table (GPA).
    /// Used by the `TableReadOps` impl for page table walking.
    root_pt_gpa: u64,
...
impl Snapshot {
    ...
    pub(crate) fn from_env<'a, 'b>(
        ...
        let pt_base_gpa = {
            ...
            // 2. Map the special mappings (scratch region + page tables)
            map_specials(&pt_buf, cfg.get_scratch_size());
    ...
    pub(crate) fn new<S: SharedMemory>(
        ...
        let memory = shared_mem.with_exclusivity(|e| e.as_slice().to_vec())?;
...
mod tests {
    ...
    fn restore() {
        ...
        let layout =
            crate::mem::layout::SandboxMemoryLayout::new(cfg, 4096, 2048, 4096, 0, None, 0)
                .unwrap();
